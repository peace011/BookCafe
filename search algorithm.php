...
1. b.
<!-- To ensure that book names are not shown in the search suggestion when the user clears the input, -->
<!-- the book names will be filtered based on the search term, and only the relevant book details will be displayed in the designated section. -->
...
2.a method without algorithm 

<!-- Search book startsWith method to check if the book name starts with the entered search term. -->
 <script type="text/javascript">
    $(document).ready(function() {
        ...
        function filterBooks(searchTerm) {
            ...
            // Loop through each book row in the table
            $('#bookList tbody tr').each(function() {
                var bookName = $(this).find('td:nth-child(2)').text().toLowerCase();
                // Check if the book name starts with the search term
                if (bookName.startsWith(searchTerm)) {
                    $(this).show();
                } else {
...
<?php } else { ?>
    <?php if ($_SESSION['obbsuid'] == "") { ?>
        <td><a href="login.php" class="btn btn-default">Book Services</a></td>
    <?php } else { ?>
        <td><a href="book-services.php?bookid=<?php echo $row->ID; ?>" class="btn btn-default">Book Services</a></td>
    <?php } ?>
<?php } ?> -->
